...
class RedisMessageBus(MessageBus):
    ...
    async def subscribe(self, channel: str, handler: Callable[[Message], None]) -> None:
        ...
        if handler not in self.subscriptions[channel]:
            self.subscriptions[channel].append(handler)
        self.logger.info(f"Subscribed to channel: {channel}")

    async def unsubscribe(self, channel: str, handler: Optional[Callable] = None) -> None:
        if channel in self.subscriptions:
            if handler is not None and handler in self.subscriptions[channel]:
                self.subscriptions[channel].remove(handler)
            if handler is None or not self.subscriptions[channel]:
    ...
    async def unsubscribe(self, channel: str, handler: Optional[Callable] = None) -> None:
        if channel in self.subscriptions:
            if handler is not None:
                if handler in self.subscriptions[channel]:
                    self.subscriptions[channel].remove(handler)
...
class CommunicationProtocol:
    ...
    def _setup_default_channels(self):
        channels = [f"agent.{self.agent_id}", "broadcast", "system.commands", "coordination"]

        try:
            loop = asyncio.get_running_loop()
            for channel in channels:
                loop.create_task(self._subscribe_channel(channel))
        except RuntimeError:
            pass

    async def _subscribe_channel(self, channel: str):
        if channel not in self._subscribed_channels:
            await self.message_bus.subscribe(channel, self._handle_message)
    ...
    async def cleanup(self):
        channels = [f"agent.{self.agent_id}", "broadcast", "system.commands", "coordination"]
        for channel in channels:
            await self.message_bus.unsubscribe(channel, self._handle_message)
    ...
    async def send_request(
        self, recipient_id: str, message_type: MessageType, payload: Dict[str, Any], timeout: float = 30.0
    ) -> Message:
        ...
        try:
            await self.send_message(
                recipient_id=recipient_id, message_type=message_type, payload=payload, correlation_id=correlation_id
            )
    ...
    async def send_heartbeat(self, status_info: Dict[str, Any]):
        await self.broadcast_message(
            ...
            {"agent_id": self.agent_id, "status": status_info, "timestamp": datetime.now(timezone.utc).isoformat()},
        )
...
class CoordinationService:
    def __init__(self, message_bus: MessageBus):
        ...
        self.protocol.register_handler(MessageType.HEARTBEAT, self._handle_agent_heartbeat)
        self.protocol.register_handler(MessageType.AGENT_STATUS, self._handle_agent_status)
    ...
    async def assign_task(self, task_id: str, task_data: Dict[str, Any], target_agent: str = None) -> bool:
        if target_agent:
    ...
    def _select_best_agent(self, task_data: Dict[str, Any]) -> Optional[str]:
        ...
        return min(
            available_agents, key=lambda agent_id: self.active_agents[agent_id].get("status", {}).get("queue_size", 0)
        )

    async def broadcast_system_command(self, command: str, params: Dict[str, Any]):
        await self.protocol.broadcast_message(MessageType.SYSTEM_COMMAND, {"command": command, "params": params})
